...
function gen_C0_C1_fixedT ( ramanshift , laser_nm , wl_wave ,   norm_pnt , temperatureK , [ maskWave , set_mask_nan] )
	...
	wave wl_wave			// white_light spectra, un-normalized, 1D or 2D, vector
	variable norm_pnt		// normalization index, pnt, scalar
	variable temperatureK	// temperature of the lamp
	...
	if( ParamIsDefault(maskWave) ) 
		...
		print "\t Mask wave not supplied. Working with full wave.\r"
		result = gen_C1_fixedT(ramanshift, laser_nm ,  wl_norm ,  norm_pnt, temperatureK)

	elseif ( waveExists ( maskWave ))
		print "\tMask wave is supplied. Using it for fit.\r"
		result = gen_C1_mask_fixedT (ramanshift, laser_nm ,  wl_norm ,   norm_pnt , maskWave, temperatureK )
	endif	
...
Function gen_C1_fixedT(ramanshift, laser_nm ,  wl_input   norm_pnt, temperatureK)
	...
	// defining initial coefs
	coefs[0] = 0.856e-18
	coefs[1] = temperatureK 
	
		
	Display /K=1 wl_input vs abs_wavenumber
	
	//perform fitting  (temperature is fixed)
	FuncFit /H="01" photons_per_unit_wavenum_abs coefs wl_input /X=abs_wavenumber 
...
Function gen_C1_mask_fixedT (ramanshift, laser_nm ,  wl_input ,   norm_pnt , maskWave, temperatureK )
	wave ramanshift		// relative
	variable laser_nm		// laser wavelength (nm)
	wave wl_input		// normalized wl wave with C0 multiplied
	variable norm_pnt		// normalization index, pnt
	...
	// perform fitting, with mask applied (temperature is fixed)
	FuncFit /H="01" photons_per_unit_wavenum_abs coefs wl_input /X=abs_wavenumber /M=maskWave
